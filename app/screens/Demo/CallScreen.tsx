...
export default class CallScreen extends Component {
  ...
  componentDidMount(): void {
    ...
    if (this.state.isIncoming) {
      this.call.current.initAnswer(this.state.callId, (status, code, message) => {
  ...
  switchPress = () => {
    this.call.current.switchCamera(this.state.callId, (status, code, message) => {
      console.log("switch - " + message)
    })
  }

  mutePress = () => {
    this.call.current.mute(this.state.callId, !this.state.isMute, (status, code, message) => {
      if (status) {
        this.setState({
  ...
  render(): React.ReactNode {
    const CircleBtn = ({ color, iconName, iconColor, onPress }) => (
      <TouchableOpacity
        ...
        onPress={onPress}
      >
        <Icon icon={iconName} type="material" color={iconColor} size={30} />
      </TouchableOpacity>
    )
